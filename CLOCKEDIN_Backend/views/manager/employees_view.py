...
class ManageEmployeesView(ViewSet):
    ...
    def assign_user_to_manager(self, request):
        ...
        return Response(
            {"detail": "User not found or already assigned to a manager."}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def detach_user_from_manager(self, request):
        ...
        return Response(
            {"detail": "User not found or not assigned to this manager."}, status=status.HTTP_400_BAD_REQUEST
        )
